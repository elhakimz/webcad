...
module threaded_bolt(thread_d=10, thread_id=8, thread_len=30, pitch=1.5, type="V", hex_d=15, head_h=6) {
    ...
    color("crimson") {
        translate([0, 0, -thread_len]) {
            threaded_rod(d=thread_d, id=thread_id, length=thread_len, pitch=pitch, type=type);
        }
    }
    ...
    color("teal") {
        translate([0, 0, 0]) {
            intersection() {
                ...
                // Cone cutter for proportional top-bevel
                cylinder(r1=hex_d * 0.75, r2=hex_d * 0.55, h=head_h + 0.05, center=false, $fn=32);
            }
        }
    }
}
