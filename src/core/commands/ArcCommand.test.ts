...
describe('ArcCommand', () => {
  it('should transition through 3 points and create an Arc', () => {
    ...
    // Step 0: Start point
    const res1 = cmd.onPoint(0, 0, 'A1', { type: 'decimal', precision: 2, scale: 1.0 })
    expect(cmd.step).toBe(1)
    expect(res1).toContain('P1[X:0.00, Y:0.00, Z:0.00]')

    // Step 1: Second point
    const res2 = cmd.onPoint(10, 0, 'A1', { type: 'decimal', precision: 2, scale: 1.0 })
    expect(cmd.step).toBe(2)
    expect(res2).toContain('P2[X:10.00, Y:0.00, Z:0.00]')


    // Step 2: End point
    const res3 = cmd.onPoint(10, 10, 'A1', { type: 'decimal', precision: 2, scale: 1.0 })
  ...
  it('should handle collinear points by staying on step 0', () => {
    ...
    cmd.onPoint(100, 0, 'A1', { type: 'decimal', precision: 2, scale: 1.0 })
    const res = cmd.onPoint(200, 0, 'A1', { type: 'decimal', precision: 2, scale: 1.0 })
  ...
  it('should provide a preview in step 2', () => {
    const cmd = new ArcCommand()
    cmd.onPoint(0, 0, 'A1', { type: 'decimal', precision: 2, scale: 1.0 })
    cmd.onPoint(100, 100, 'A1', { type: 'decimal', precision: 2, scale: 1.0 })
    
    const preview = cmd.getPreview(200, 0, { type: 'decimal', precision: 2, scale: 1.0 })
