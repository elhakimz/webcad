...
export class DimLinearCommand implements Command {
  ...
  onPoint(x: number, y: number, _id: string, _units: UnitsConfig): CommandResponse {
    ...
    if (this.step === 2) {
      this.dimLinePt = { x, y };
      const id = _id || "DIM";
      const dim = new Dimension(id, "LINEAR", this.p1!.x, this.p1!.y, this.p2!.x, this.p2!.y, 10);
  ...
  getPreview(x: number, y: number, _units: UnitsConfig): PreviewObject | null {
    if (this.step === 2 && this.p1 && this.p2) {
      const dim = new Dimension("PREVIEW", "LINEAR", this.p1.x, this.p1.y, this.p2.x, this.p2.y, 10);
