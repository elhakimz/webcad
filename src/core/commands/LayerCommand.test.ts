...
describe('LayerCommand', () => {
  it('should return action:finish on exit', () => {
    ...
    expect(cmd.onInput('', 'DUMMY', { type: 'decimal', precision: 2, scale: 1.0 })).toEqual({ type: 'action', action: 'finish' })
  })
  ...
  it('should handle NEW option with two-step interaction', () => {
    ...
    expect(res1).toEqual({ type: 'prompt', text: 'New layer name:' })
    ...
    expect(res2).toEqual({ type: 'action', action: 'layerNew', name: 'TEST_LAYER' })
  ...
  it('should handle ON option with two-step interaction', () => {
    ...
    expect(res).toEqual({ type: 'action', action: 'layerOn', names: '0,TEST' })
  })
  ...
  it('should handle LTYPE option with numerical layer name', () => {
    ...
    expect(res).toEqual({ type: 'action', action: 'layerLinetype', linetype: 'DASHED', names: '01' })
  })
})
