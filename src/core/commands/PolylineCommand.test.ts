...
describe('PolylineCommand', () => {
  it('should support continuous vertices', () => {
    ...
    cmd.onPoint(0, 0, 'PL1')
    const pline1 = cmd.onPoint(10, 10, 'PL1') as Polyline
    ...
    const pline2 = cmd.onPoint(20, 10, 'PL1') as Polyline
  ...
  it('should support Close (C)', () => {
    const cmd = new PolylineCommand()
    cmd.onPoint(0, 0, 'PL1')
    cmd.onPoint(10, 0, 'PL1')
    cmd.onPoint(10, 10, 'PL1')
    
    const res = cmd.onInput('C', 'PL1') as CommandAction
  ...
  it('should support Arc mode with tangency', () => {
    const cmd = new PolylineCommand()
    cmd.onPoint(0, 0, 'PL1')
    cmd.onPoint(2, 0, 'PL1') // Line from 0,0 to 2,0. Tangent is 0 deg.
    
    cmd.onInput('A', 'PL1') // Switch to Arc mode
