...
describe("Linear Bearings Showcase Test", () => {
  it("should successfully parse and evaluate the linear bearings showcase", async () => {
    const rootDir = path.join(__dirname, "../../files/scad/projects/myproject");
    const code = fs.readFileSync(path.join(rootDir, "linear_bearings_showcase.scad"), "utf-8");
    ...
    console.log("Successfully evaluated linear bearings showcase. Geometry nodes: ", geometryTree.length);
  });
});
