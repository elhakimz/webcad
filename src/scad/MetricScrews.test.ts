...
describe("Metric Screws Showcase Test", () => {
  it("should successfully parse and evaluate the metric screws showcase", async () => {
    const rootDir = path.join(__dirname, "../../files/scad/projects/myproject");
    const code = fs.readFileSync(path.join(rootDir, "metric_screws_showcase.scad"), "utf-8");
    ...
    console.log("Successfully evaluated metric screws showcase. Geometry nodes: ", geometryTree.length);
  });
});
