...
describe("Torx Drive Showcase Test", () => {
  it("should successfully parse and evaluate the torx drive showcase", async () => {
    const rootDir = path.join(__dirname, "../../files/scad/projects/myproject");
    const code = fs.readFileSync(path.join(rootDir, "torx_drive_showcase.scad"), "utf-8");
    ...
    console.log("Successfully evaluated torx drive showcase. Geometry nodes: ", geometryTree.length);
  });
});
