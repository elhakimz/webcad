...
describe("Wiring Showcase Test", () => {
  it("should successfully parse and evaluate the wiring bundle showcase with multiple side profiles", async () => {
    const rootDir = path.join(__dirname, "../../files/scad/projects/myproject");
    const code = fs.readFileSync(path.join(rootDir, "wiring_showcase.scad"), "utf-8");
    ...
    console.log("Successfully evaluated wiring showcase with multiple custom profiles. Geometry nodes: ", geometryTree.length);
  });
});
