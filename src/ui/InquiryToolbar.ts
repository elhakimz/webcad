...
export class InquiryToolbar {
  ...
  constructor(private onAction: (cmd: string) => void, private dockingManager?: DockingManager) {
    ...
    if (this.dockingManager) {
      ...
      this.dockingManager.registerWindow('inquiry_toolbar', this.container, true); 
    }
  }

  private createToolbar() {
    this.container = document.createElement('div');
    this.container.id = 'inquiry-toolbar';
    ...
    this.headerEl.innerHTML = `
      ...
      <div class="toolbar-controls">
        <span class="control-btn" id="inquiry-tb-dock">[ ]</span>
        <span class="control-btn" id="inquiry-tb-minimize">_</span>
      </div>
    `;
    ...
    // Dock behavior
    const dockBtn = this.headerEl.querySelector('#inquiry-tb-dock')!;
    dockBtn.addEventListener('click', () => {
      this.dockingManager?.toggleDock('inquiry_toolbar');
    });

    // Minimize behavior
    const minimizeBtn = this.headerEl.querySelector('#inquiry-tb-minimize')!;
  ...
  private populateCommands() {
    const commands = [
      { cmd: 'DIST', icon: 'measure.svg', title: 'Distance' },
      { cmd: 'AREA', icon: 'polygonal_area.svg', title: 'Area' },
      { cmd: 'ID', icon: 'pick_coordinates.svg', title: 'ID Point' },
      { cmd: 'LIST', icon: 'entity_description_info.svg', title: 'List' }
    ];
